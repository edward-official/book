...
## 왜 가상주소 0x0번지 근처는 매핑이 금지되어 있을까?
핵심은 대부분의 프로그래밍의 언어에서 `NULL == 0x0`이라는 사실이다.
이게 어떤 의미가 있을까?
...
다음과 같은 버그 상황은 반드시 segmentation fault로 처리되어야하는 오류이다.
그런데 만약 이 주소가 어떤 물리 페이지와 매핑되어 있다면 반드시 폴트를 발생기켜야하는 상황에서 폴트 없이 정상 동작해버리는 경우가 생길 수 있다.
따라서 운영체제는 이 주소의 일정 범위를 매핑 금지 영역으로 설정해 NULL 접근을 확실하게 잡아내려고 한 것이다.
이런 정책을 주로 **Low Memory Protection** 또는 **Low Address Space Protection**이라고 부른다.

#### 갑자기 궁금해진 segmentation fault와 page fault의 차이점
결론부터 말하자면 너무 간단한데, 예를 들어 page fault는 특정 페이지가 아직 메모리에 로드되지 않아서 CPU가 접근하려는 주소의 페이지 테이블 엔트리가 없는 경우이고, segmentation fault는 전혀 잘못된 메모리에 접근해서 그냥 실행중인 흐름을 종료해야하는 상황이라고 할 수 있다.

즉, page fault는 정상적으로 복구 가능한 상황일 수도, 그렇지 않을 수도 있지만 segmentation fault는 명백히 복구 불가능한 상황인 것이다.
